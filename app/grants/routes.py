...
Endpoints:
  GET    /api/grants                  — List grants (filtered by council for scoped roles)
...
def list_grants(current_user):
    ...
    if current_user.role != "system_admin":
        ...
        # Community members and consultants only see published grants
...
def get_grant(current_user, grant_id):
    ...
    # Community / consultant only see published grants
...
def create_grant(current_user):
    ...
    grant = Grant(
        ...
        max_amount_per_application = data.get("max_amount"),
        min_amount_per_application = data.get("min_amount"),
        opens_at    = opens_at,
        closes_at   = closes_at,
        assessment_deadline = datetime.fromisoformat(data["assessment_deadline"]) if data.get("assessment_deadline") else None,
        notification_date   = datetime.fromisoformat(data["notification_date"])   if data.get("notification_date")   else None,
        status      = "draft",
        is_published= False,
        allow_multiple_applications = data.get("allow_multiple_applications", False),
        require_community_voting    = data.get("require_community_voting", False),
        enable_mapping              = data.get("enable_mapping", False),
